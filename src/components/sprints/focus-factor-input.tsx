...
/**
 * Project-wide focus factor input. A single percentage that is applied to
 * every sprint. Kept next to the Add Sprint button because focus is a
 * planning assumption, not a per-sprint attribute in practice.
 */
export function FocusFactorInput({ initial }: { initial: number }) {
  ...
  async function commit() {
    ...
    setSaving(true);
    try {
      const res = await fetch("/api/sprints/focus-factor", {
        method: "POST",
  ...
  return (
    <div
      ...
      title="Global focus factor — applied to every sprint's capacity projection"
    >
